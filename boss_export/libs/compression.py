...
from boss_export.libs.exceptions import DecompressionError, CompressionError
...
def decompress(content, encoding, filename="N/A"):
  ...
  """
    ...
    raise NotImplementedError(
        str(encoding) + " is not currently supported. Supported Options: None, gzip"
    )
...
def compress(content, method="gzip"):
  ...
  """
    ...
    raise NotImplementedError(
        str(method) + " is not currently supported. Supported Options: None, gzip"
    )
  ...
  """
    ...
    if first_two_bytes != gzip_magic_numbers:
        raise DecompressionError(
            "File is not in gzip format. Magic numbers {}, {} did not match {}, {}.".format(
                hex(first_two_bytes[0]), hex(first_two_bytes[1])
            ),
            hex(gzip_magic_numbers[0]),
            hex(gzip_magic_numbers[1]),
        )
